...
public class YAwaitAccuseFragment extends BaseFragment implements
...
//                if (accuseAdapter.getData().get(position).getIsUrgent() != null) {
//                    if (accuseAdapter.getData().get(position).getIsUrgent().equals("0")) {
//                        msg("当前方案暂无质控信息!");
//                    } else {
//                        AwaitAccuseActivity_.intent(getActivity()).schemeid(accuseAdapter.getData().get(position).getSchemeid()).start();

                startActivity(new Intent(getActivity(), AccuseAllinfoActivity.class).putExtra("id", accuseAdapter.getData().get(position).getId()).putExtra("schid", accuseAdapter.getData().get(position).getSchemeid()));
//                    }
//                }
            }
        });
    ...
    public void initData(int page, String querystring) {
        String data = "{id:\"\",projectid:\"\",schemeid:\"\",querystring:\"" + querystring + "\",contractstatus:\"" + Contractstatus_DAIFUWEI_YIWANJIE + "\",sdate:\"\",edate:\"\",smoney:\"\",emoney:\"\",pagenumber:\"" + page + "\",isqualitycontrol:\"1\",numbers:\"" + PageNum + "\"} ";
