...
public class AccuseActivity extends BaseFragment implements SubmitView<List<ProjectcontractBean>> {
    ...
    void init() {
        ...
        accuseAdapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                startActivity(new Intent(getActivity(), AccuseInfoActivity.class).putExtra("id", accuseAdapter.getData().get(position).getId()).putExtra("schid", accuseAdapter.getData().get(position).getSchemeid()));
            }
        });
        /**
         * 下拉
    ...
    public void initData(int page, String querystring) {
        String data = "{id:\"\",projectid:\"\",schemeid:\"\",querystring:\"" + querystring + "\",contractstatus:\"" + Contractstatus_DAIFUWEI_YIWANJIE + "\",sdate:\"\",edate:\"\",smoney:\"\",emoney:\"\",pagenumber:\"" + page + "\",isqualitycontrol:\"3\",numbers:\"" + PageNum + "\"} ";
        financeMainPresenter.Getprojectcontract(data);
    }
    @Subscribe
    public void refresh(ReshEvent event) {
        if (event != null) {
            refreshLayout.autoRefresh();//自动刷新
        }
    }
...
//            refreshLayout.finishRefresh();
            ...
            if (projectcontractBeans.size() == 0) {
                onLoadMore = true;
            }
//            refreshLayout.finishRefresh();
