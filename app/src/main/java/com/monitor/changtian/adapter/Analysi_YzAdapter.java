...
public class Analysi_YzAdapter extends BaseQuickAdapter<SampleInfoByPointInfoBean.DetailsBean.FactorsBean, BaseViewHolder> {
    public Analysi_YzAdapter(int layoutResId) {
        super(layoutResId);
    }
    ...
    protected void convert(final BaseViewHolder helper, SampleInfoByPointInfoBean.DetailsBean.FactorsBean item) {
        ...
        cb_doctor.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    Analysi_YzAdapter.this.getData().get(helper.getLayoutPosition()).setChoice(true);
                } else {
                    ViseLog.d("取消选中");
                    Analysi_YzAdapter.this.getData().get(helper.getLayoutPosition()).setChoice(false);
                }
            }
        });

        if (Analysi_YzAdapter.this.getData().get(helper.getLayoutPosition()).isChoice_save()) {
