...
public class ConsumableListAdapter  extends BaseQuickAdapter<TasksInfomationBean.ConsumablesBean, BaseViewHolder> {
    public ConsumableListAdapter(int layoutResId) {
        super(layoutResId);
    }
    @Override
    protected void convert(final BaseViewHolder helper, TasksInfomationBean.ConsumablesBean item) {
        ...
        cb_doctor.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    ConsumableListAdapter.this.getData().get(helper.getLayoutPosition()).setChoice(true);
                } else {
                    ViseLog.d("取消选中");
                    ConsumableListAdapter.this.getData().get(helper.getLayoutPosition()).setChoice(false);
                }
            }
        });

        if (ConsumableListAdapter.this.getData().get(helper.getLayoutPosition()).isChoice_save()) {
