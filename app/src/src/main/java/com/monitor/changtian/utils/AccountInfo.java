...
public class AccountInfo {
    ...
    public static void saveAccount(Context ctx, AllUserInfo data) {
    ...
    public static AllUserInfo loadAccount(Context ctx) {
        AllUserInfo data = null;
        File file = new File(ctx.getFilesDir(), ACCOUNT);
        if (file.exists()) {
            try {
                ObjectInputStream ois = new ObjectInputStream(ctx.openFileInput(ACCOUNT));
                data = (AllUserInfo) ois.readObject();
                ois.close();
            } catch (Exception e) {
                e.printStackTrace();
            }
