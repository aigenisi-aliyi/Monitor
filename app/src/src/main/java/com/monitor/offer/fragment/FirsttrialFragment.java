...
public class FirsttrialFragment extends BaseFragment  implements OfferView {
    ...
    void init() {
        ...
        firsttrialAdapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                 ...
                 * 查看详情
                 */
                OfferinfoActivity_.intent(getActivity()).type("0").id(firsttrialAdapter.getData().get(position).getId()).sqstatus("0").start();
            }
        });
    }
...
//    }
    public void initData() {
        ViseLog.d("lslslsl");
        String data = "{schemeid:\"\",id:\"\",sqstatus:\"0\"}";
    ...
    public void UpdateList(UpdateEvent event) {
        if (event != null) {
            if(event.getType().equals("Firsttrial")){
                initData();
            }
        }
    }
